...
public class ContaService {
	...
	public SaldoResponse verificarSaldo(String cpf) {
		logger.info("Verificando saldo da conta: CPF {}", cpf);
		Conta conta = contaRepository.findByCpf(cpf).orElseThrow(() -> new RuntimeException("Conta não encontrada"));
		return new SaldoResponse(conta.getTitular(), conta.getCpf(), conta.getIdade(), conta.getSaldo(),
				conta.getAgencia(), conta.getNumero(), conta.getTipo());
	}
	...
	public void transferir(Integer numeroContaOrigem, Integer numeroAgenciaOrigem, Integer numeroContaDestino,
		...
		if (contaOrigem.getSaldo() < valor) {
			logger.warn("Saldo insuficiente na conta de origem: Agência {}, Número {}", numeroAgenciaOrigem,
					numeroContaOrigem);
		...
		Extrato log = new Extrato(contaOrigem.getNumero(), contaOrigem.getAgencia(), contaDestino.getNumero(),
				contaDestino.getAgencia(), Calendar.getInstance().getTime(), valor);
	...
	public ContaDto criarConta(ContaDto dto) {
		...
		if (contaRepository.existsByCpf(cpf)) {
			String eventoMessage = "Tentativa de criação de conta com CPF já existentes, CPF: " + cpf;
			Evento evento = new Evento(eventoMessage, LocalDateTime.now());
			eventoRepository.save(evento);
	...
	public void depositarNaConta(Integer numeroConta, Integer numeroAgencia, double valor) {
		...
		String eventoMessage = String.format("Depósito realizado: Conta %s, Agência %s, Valor %s", numeroConta,
				numeroAgencia, valor);
		Evento evento = new Evento(eventoMessage, LocalDateTime.now());
		eventoRepository.save(evento);
		logger.info(eventoMessage);
